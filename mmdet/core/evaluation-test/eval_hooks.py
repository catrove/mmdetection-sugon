...
class DistEvalHook(Hook):
    ...
    def after_train_epoch(self, runner):
        ...
        #prog_bar = mmcv.ProgressBar(int(len(self.dataset) / runner.world_size) + 1)
        ...
        if runner.rank == 0:
            ...
            #self.evaluate_test(runner, results)
        else:
...
class DistEvalmAPHook(DistEvalHook):
    ...
    def evaluate(self, runner, results):
        ...
        for i in range(len(self.dataset)):
            ...
            #bboxes = ann['bboxes']
            #labels = ann['labels']
            bboxes = np.array(ann['bboxes'], dtype='float32')
            labels = np.array(ann['labels'], dtype='int64')
...
class CocoDistEvalmAPHook(DistEvalHook):
    ...
    def evaluate_test(self, runner, results):
