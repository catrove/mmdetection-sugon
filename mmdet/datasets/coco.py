...
from .train_cat_id import _cat_ids
...
class CocoDataset(CustomDataset):
    ...
    def load_annotations(self, ann_file):
        ...
        #self.cat_ids = list(range(366)) #self.coco.getCatIds()
        self.cat2label = {
            cat_id: i + 1
            for i, cat_id in enumerate(self.cat_ids)
        }
        print("##coco #%%% id len is ", len(self.cat_ids))
    ...
    def _parse_ann_info(self, ann_info, with_mask=True):
        ...
        if gt_bboxes:
            ...
            #gt_bboxes = np.array(gt_bboxes_ignore, dtype=np.float32)
            #gt_labels = np.array(gt_labels_ignore, dtype=np.int64)
