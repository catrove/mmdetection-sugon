...
class Centripetal_mask(nn.Module):
    ...
    def _init_layers(self):
        ...
        if self.with_mask:
            ...
            self.conv_logits = nn.Conv2d(self.in_channels, self.num_classes+1, 1)
            self.mid_conv_logits = nn.Conv2d(self.in_channels, self.num_classes+1, 1)
    ...
    def forward_single(self, feats):
        ...
        #print(" forward t1 poll #####")
        tl_pool = self.tl_pool(x)
        #print(" forward t1 heat #####")
        tl_heat = self.tl_heat(tl_pool)
        #print(" forward t1 off_c #####")
        tl_off_c = self.tl_off_c(tl_pool)
        #print(" forward t1 off #####")
    ...
    def loss(self, tl_result, br_result, mask, mid_tl_result, mid_br_result, mid_mask, gt_bboxes, gt_labels, gt_masks, img_metas, cfg, imgscale):
        ...
        gt_tl_off_c2, gt_br_off_c2 = corner_target(gt_bboxes=gt_bboxes, gt_labels=gt_labels, feats=tl_result, imgscale=imgscale, direct=True, scale=1.0, dcn=True, num_classes=self.num_classes)
    ...
    def get_bboxes(self, tl_result, br_result, mask, mid_tl_result, mid_br_result, mid_mask, img_metas, cfg, rescale=False):
        ...
        for img_id in range(len(img_metas)):
            ...
            #print("scores_n length ", scores_n.size())
