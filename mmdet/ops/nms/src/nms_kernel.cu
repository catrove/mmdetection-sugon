...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  mask_dev = (unsigned long long*) hipMalloc(state, boxes_num * col_blocks * sizeof(unsigned long long));

  dim3 blocks(CEILDIV(boxes_num, threadsPerBlock),
              CEILDIV(boxes_num, threadsPerBlock));
  dim3 threads(threadsPerBlock);
  hipLaunchKernelGGL(nms_kernel, dim3(blocks), dim3(threads), 0, at::cuda::getCurrentCUDAStream(), boxes_num,
                                  nms_overlap_thresh,
                                  boxes_dev,
                                  mask_dev);
